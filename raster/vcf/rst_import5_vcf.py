...
# hdf layer names
lyrnames = ['Percent_Tree_Cover', 'Percent_NonTree_Vegetation',
...
def work_merge(fnames, workdir, dryrun=False):
    ...
    for fname in fnames:
        ...
        cmd = "gdal_merge.py -separate -o %(tifname)s -co 'COMPRESS=LZW' -co 'BIGTIFF=YES' -co 'TILED=YES' %(sdsnames)s" % dict(
                tifname=tifname,
...
def main(year):
    ...
    fnames = sorted(glob.glob("%(ddir)s/%(year)s.03.0[56]/MOD44B.A%(year)s065.h??v??.006.*.hdf" % dict(
            ddir = ddir, year=year)))
    bname = 'MOD44B.A%(year)s065' % dict(year=year)
    ...
    dir_proc = os.path.join(workdir, 'prc')
    if True:
        # do the math?
        prcnames = work_math(mrgnames, dir_proc)
    else:
        prcnames = sorted(glob.glob(os.path.join(dir_proc, '*.tif')))

    # resample
    dir_rsmp = os.path.join(workdir, 'rsp')
    if True:
        rsmpnames = work_resample_pieces(prcnames, dir_rsmp, bname)
