...
#    cmd += tifnames
    # with anaconda on PC, this long list of file names choked the OS
    # so i assume that i use all tif in the same dir
    mytifnames = os.path.join(os.path.dirname(tifnames[0]), '*.tif')
    cmd += [mytifnames]
    cmd += [dstname]
    print(cmd)
...
def work_merge(fnames, workdir, dryrun=False):
    ...
    for fname in fnames:
        ...
        cmd = "gdal_merge.py -separate -o %(tifname)s %(tiffopt)s %(sdsnames)s" % dict(
                tifname=tifname,
                tiffopt=tiffopt,
