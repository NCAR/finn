...
-- identify which point to be skimmed
-- also specify group of points which is to be replaced by centroid, not just dropped.
...
#            plpy.notice('here0')
            if xx == 999: plpy.error(xx)
            if cc.size() == 0: break
            assert ccs > cc.size()
#            plpy.notice('here1')
            # for each node calculate sum of invdist
            # and find node with the maximum
            m = 0
            mydrop = []
	    # adjajceny_iter() to adjacency(), changed in networkx 2.0
            #for n, nbr in cc.adjacency_iter():
            ...
            # if there are ties, they are replaced by the mid point of subgraphs
            # replace may come as a chunk, so need to look into connectedness
...
create or replace function st_voronoi_py_old(pnts geometry)
returns geometry as
$$
...
select st_setsrid(st_collect(geom), st_srid(pnts)) from (
-- 	select st_makepolygon(st_makeline(geom)) as geom from (
	select st_makevalid(st_makepolygon(st_makevalid(st_makeline(geom)))) as geom from (
		select st_point(x, y) as geom, idx from (
			select unnest((baz.coords).x) as x, unnest((baz.coords).y) as y, (baz.coords).pos as idx from (
				select st_voronoi_python(x, y) as coords  from (
					select array_agg(st_x(geom)) as x, array_agg(st_y(geom)) as y from (
						select (st_dump(pnts)).geom) as foo
					) as bar group by 1
			) as baz 
		) as qux
	) as boz group by idx order by idx
) as thud;
    ...
    if len(p) == 2:
        ...
        if cos < -.999 or cos > .999:
            ...
            for i in 1,2:
                ...
                lst.append(        ( [far_points[:,_].tolist() for _ in (0,1)] + [i+1] ) )
                n.append(normal) 
            ...
            if is_obtuse:
                lst = [
                    [[center0[_], far_points0[1][_],                     far_points0[2][_],center0[_]] for _ in (0,1)] + [0],
                    [[center0[_], far_points0[2][_], side_points0[2][_], far_points0[0][_],center0[_]] for _ in (0,1)] + [1],
                    [[center0[_], far_points0[0][_], side_points0[1][_], far_points0[1][_],center0[_]] for _ in (0,1)] + [2],
                    ]
            else:
                lst = []
                for i in range(3):
                    lst.append([[center0[_],far_points0[(i+1) % 3][_],far_points0[(i+2) % 3][_],center0[_]] for _ in (0,1)] + [i+1])
            #plpy.notice(lst)
            #raise
            #plpy.notice("lst: %s" % lst)
...
create or replace function st_cutter_py_old(pnts geometry)
returns geometry as
$$
select st_setsrid(st_collect(geom), st_srid(pnts)) from (
-- 	select st_makepolygon(st_makeline(geom)) as geom from (
	select st_makevalid(st_makepolygon(st_makevalid(st_makeline(geom)))) as geom from (
		select st_point(x, y) as geom, idx from (
			select unnest((baz.coords).x) as x, unnest((baz.coords).y) as y, (baz.coords).pos as idx from (
				select st_cutter_python(x, y) as coords  from (
					select array_agg(st_x(geom)) as x, array_agg(st_y(geom)) as y from (
						select (st_dump(pnts)).geom) as foo
					) as bar group by 1
			) as baz 
...
-- grab relevent fields
insert into work_pnt  (rawid, geom_pnt, lon, lat, scan, track, acq_date, confidence) select
...
-- dup tropics
insert into work_pnt (rawid, geom_pnt, lon, lat, scan, track, acq_date, confidence)
select rawid, geom_pnt, lon, lat, scan, track, acq_date + 1, confidence from work_pnt
where abs(lat) <= 30
;
...
do language plpgsql $$ begin
raise notice 'tool: cleaned, %', clock_timestamp();
end $$;
