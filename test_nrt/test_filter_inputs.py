...
df_txt = {k: [gpd.read_file((ddir / 'tmp'/ fname).with_suffix('.shp')) for fname in fnames] for k, fnames in txtfiles.items()}
...
#
#for k in 'm', 'v':
#    ds0 = drv.Open(str(ddir / (shpfiles[k])))
#    lyr0= ds0.GetLayer()
#    srs = lyr0.GetSpatialRef()
#
#    ds = drv.CreateDataSource( str(ddir / (shpfiles[k][:-4] + '_filtered.shp')))
#        ds.CreateLayer(shpfiles[k][:-4] + '_filtered', lyr0.GetSpatialRef(), lyr0.
#    srs = ds0.
#
#
#
